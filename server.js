  ...
  if (req.method === 'POST' && req.url === '/facturar') {
    ...
    req.on('end', async () => {
      ...
      try {
        ...
        // Guardamos el comprobante emitido — sea cual sea el origen (Modo
        // POS, venta cargada a mano, etc.), queda un registro fiscal propio
        await sb.schema('ops').from('facturas_emitidas').insert({
